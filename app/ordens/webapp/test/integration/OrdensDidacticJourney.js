...
sap.ui.define([
    ...
    function journey() {
        ...
        opaTest("filtra uma ordem pelo código", function (Given, When, Then) {
            When.onTheOrdensList.onFilterBar().iChangeFilterField(CODE_FILTER, "OM-0001", true);
            When.onTheOrdensList.onFilterBar().iExecuteSearch();

            Then.onTheOrdensList.onFilterBar().iCheckFilterField(CODE_FILTER, "OM-0001");
            Then.onTheOrdensList.onTable().iCheckRows({ Ordem: "OM-0001" }, 1);
        });
         ...
         */
        opaTest("abre a ordem e apresenta suas reservas", function (Given, When, Then) {
            When.onTheOrdensList.onTable().iPressRow({ Ordem: "OM-0001" });

            Then.onTheOrdensObjectPage.iSeeThisPage();
            Then.onTheOrdensObjectPage.onHeader().iCheckTitle(
                "OM-0001",
            ...
            Then.onTheOrdensObjectPage.onForm(GENERAL_DATA_FORM).iCheckField(
                STATUS_FIELD,
                "ABERTA"
            );
            Then.onTheOrdensObjectPage.onTable({ property: "reservas" }).iCheckRows({}, 2);
        });
         ...
         */
        opaTest("navega para uma reserva e retorna à ordem", function (Given, When, Then) {
            When.onTheOrdensObjectPage.onTable({ property: "reservas" }).iPressRow(0);
            Then.onTheReservasMateriaisObjectPage.iSeeThisPage();
         ...
         */
        opaTest("libera uma ordem com estoque suficiente", function (Given, When, Then) {
         ...
         */
        opaTest("seleciona a ordem com risco de estoque", function (Given, When, Then) {
            When.iNavigateBack();
            Then.onTheOrdensList.iSeeThisPage();

            When.onTheOrdensList.onFilterBar().iChangeFilterField(CODE_FILTER, "OM-0002", true);
            When.onTheOrdensList.onFilterBar().iExecuteSearch();
            Then.onTheOrdensList.onTable().iCheckRows({ Ordem: "OM-0002" }, 1);

            When.onTheOrdensList.onTable().iPressRow({ Ordem: "OM-0002" });
            Then.onTheOrdensObjectPage.onHeader().iCheckTitle(
                "OM-0002",
                "Substituicao de filtro hidraulico"
            );
            Then.onTheOrdensObjectPage.onForm(GENERAL_DATA_FORM).iCheckField(
                STATUS_FIELD,
                "ABERTA"
         ...
         */
        opaTest("exibe o erro de estoque insuficiente", function (Given, When, Then) {
            When.onTheOrdensObjectPage.onHeader().iExecuteAction("Liberar ordem");
            Then.onTheOrdensObjectPage.onErrorDialog().iCheckState({ visible: true });

            When.onTheOrdensObjectPage.onErrorDialog().iClose();
            Then.onTheOrdensObjectPage.iSeeThisPage();
            Then.onTheOrdensObjectPage.onForm(GENERAL_DATA_FORM).iCheckField(
                STATUS_FIELD,
                "ABERTA"
         ...
         */
        opaTest("cancela a ordem com motivo obrigatório", function (Given, When, Then) {
            When.onTheOrdensObjectPage.onHeader().iExecuteAction("Cancelar ordem");
            Then.onTheOrdensObjectPage.onActionDialog().iCheckDialogField(
                { property: "motivo" },
                ""
            );

            When.onTheOrdensObjectPage.onActionDialog().iChangeDialogField(
                { property: "motivo" },
                "Cancelamento validado pela jornada OPA5",
                true
            );
            Then.onTheOrdensObjectPage.onActionDialog().iCheckConfirm({ enabled: true });
            ...
            Then.onTheOrdensObjectPage.onForm(PLANNING_FORM).iCheckField(
                OBSERVATION_FIELD,
         ...
         */
        opaTest("inicia a criação de uma ordem em draft", function (Given, When, Then) {
            When.iNavigateBack();
            Then.onTheOrdensList.iSeeThisPage();
