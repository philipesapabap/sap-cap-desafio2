 ...
 * Teste 15 - exige motivo no cancelamento.
 * Teste 16 - cancela ordem e resolve o texto do status.
 * Teste 17 - rejeita cancelamento de ordem não aberta.
 * Teste 18 - rejeita cancelamento de ordem inexistente.
 * Teste 19 - processa lote integralmente válido.
 * Teste 20 - continua lote após falha funcional de um item.
 * Teste 21 - usa o status contratual para lote com erro.
 * Teste 22 - mantém atomicidade do item que falha no lote.
 * Teste 23 - rejeita lote já processado.
 * Teste 24 - rejeita lote aberto sem itens pendentes.
 * Teste 25 - rejeita lote inexistente.
 * Teste 26 - ativa um draft válido.
 * Teste 27 - rejeita valor estimado negativo na ativação do draft.
 */
...
describe("PlanejamentoService — fluxos HTTP com SQLite", () => {
  ...
  it("rejeita liberação de ordem não aberta", async () => {
    await expectRequestError(
      POST(actionUrl("Ordens", IDS.released, "liberarOrdem"), {}, AUTH.authorized),
      409,
      "não está aberta",
    );
  ...
  it("exige motivo no cancelamento", async () => {
    await expectRequestError(
      POST(actionUrl("Ordens", IDS.success, "cancelarOrdem"), {}, AUTH.authorized),
      400,
      "missing value",
    );
  });
  ...
  it("processa lote integralmente válido", async () => {
    ...
    expect(items.every(({ status_code }) => status_code === "SUCESSO")).to.equal(true);
  });
  ...
  async function cleanupFixtures() {
    ...
    await db.run(DELETE.from(entities.MovimentosEstoque).where({ ordem_ID: { in: orderIds } }));
    await db.run(DELETE.from(entities.ItensLoteLiberacao).where({ lote_ID: { in: lotIds } }));
    await db.run(DELETE.from(entities.LotesLiberacao).where({ ID: { in: lotIds } }));
    await db.run(DELETE.from(entities.ResponsabilidadesOrdem).where({ ordem_ID: { in: orderIds } }));
    await db.run(DELETE.from(entities.ReservasMateriais).where({ ordem_ID: { in: orderIds } }));
    await db.run(DELETE.from(entities.Ordens).where({ ID: { in: orderIds } }));
    await db.run(DELETE.from(entities.Estoques).where({ ID: { in: [IDS.stockA, IDS.stockB, IDS.stockC] } }));
    await db.run(DELETE.from(entities.Materiais).where({ ID: { in: [IDS.materialA, IDS.materialB, IDS.materialC] } }));
    await db.run(DELETE.from(entities.LocaisInstalacao).where({ ID: IDS.location }));
    ...
    await db.run(DELETE.from(entities.Usuarios).where({ matricula: { in: Object.values(USERS) } }));
  }

  async function seedFixtures() {
    await db.run(INSERT.into(entities.Usuarios).entries([
      { matricula: USERS.authorized, nome: "Usuário autorizado", ativo: true },
      { matricula: USERS.other, nome: "Outro usuário", ativo: true },
    ]));
    await db.run(INSERT.into(entities.Centros).entries({
      ID: IDS.center,
      codigo: "TC01",
      nome: "Centro de teste",
    }));
    await db.run(INSERT.into(entities.LocaisInstalacao).entries({
      ID: IDS.location,
      codigo: "TL01",
      nome: "Local de teste",
    ...
    await db.run(INSERT.into(entities.Ordens).entries([
      ...
      buildOrder(IDS.released, "T-LIBERADA", USERS.authorized, { status_code: "LIBERADA" }),
      buildOrder(IDS.successSecond, "T-SUCESSO-2", USERS.authorized),
      buildOrder(IDS.partial, "T-PARCIAL", USERS.authorized),
    ]));
    await db.run(INSERT.into(entities.ReservasMateriais).entries([
      buildReservation("b1000000-0000-4000-a000-000000000001", IDS.success, IDS.materialA, 2),
      buildReservation("b1000000-0000-4000-a000-000000000002", IDS.success, IDS.materialB, 1),
      buildReservation("b2000000-0000-4000-a000-000000000001", IDS.risk, IDS.materialC, 3),
      buildReservation("b4000000-0000-4000-a000-000000000001", IDS.otherUser, IDS.materialA, 1),
      buildReservation("b6000000-0000-4000-a000-000000000001", IDS.successSecond, IDS.materialA, 1),
      buildReservation("b7000000-0000-4000-a000-000000000001", IDS.partial, IDS.materialA, 2),
      buildReservation("b7000000-0000-4000-a000-000000000002", IDS.partial, IDS.materialC, 3),
    ]));
    await db.run(INSERT.into(entities.ResponsabilidadesOrdem).entries([
      buildResponsibility("c3000000-0000-4000-a000-000000000001", IDS.success, USERS.authorized),
      buildResponsibility("c3000000-0000-4000-a000-000000000002", IDS.risk, USERS.authorized),
      buildResponsibility("c3000000-0000-4000-a000-000000000003", IDS.withoutReservations, USERS.authorized),
      buildResponsibility("c3000000-0000-4000-a000-000000000004", IDS.otherUser, USERS.other),
      buildResponsibility("c3000000-0000-4000-a000-000000000005", IDS.released, USERS.authorized),
      buildResponsibility("c3000000-0000-4000-a000-000000000006", IDS.successSecond, USERS.authorized),
      buildResponsibility("c3000000-0000-4000-a000-000000000007", IDS.partial, USERS.authorized),
    ]));
    await db.run(INSERT.into(entities.LotesLiberacao).entries([
    ...
    await db.run(INSERT.into(entities.ItensLoteLiberacao).entries([
      buildLotItem("d8000000-0000-4000-a000-000000000001", IDS.lotSuccess, IDS.success),
      buildLotItem("d8000000-0000-4000-a000-000000000002", IDS.lotSuccess, IDS.successSecond),
      buildLotItem("d8000000-0000-4000-a000-000000000003", IDS.lotMixed, IDS.success),
      buildLotItem("d8000000-0000-4000-a000-000000000004", IDS.lotMixed, IDS.risk),
      buildLotItem("d8000000-0000-4000-a000-000000000005", IDS.lotPartial, IDS.partial),
      buildLotItem("d8000000-0000-4000-a000-000000000007", IDS.lotProcessed, IDS.success),
      buildLotItem(
  ...
  function buildMaterial(ID, codigo) {
    return { ID, codigo, descricao: codigo, unidade: "UN", precoMedio: 10, ativo: true };
  }
  ...
  function buildReservation(ID, ordem_ID, material_ID, quantidadeNecessaria) {
    return { ID, ordem_ID, material_ID, deposito_ID: IDS.deposit, quantidadeNecessaria };
  }
