...
- 2026-08-11: Validação inicial da proposta do F010. O handler específico de drafts compilou e eliminou o 404 na ativação, mas o teste `ativa um draft válido` ainda falhou: a action respondeu 204 em vez de 201 porque a ordem ativa recém-criada continua sem registro em `ResponsabilidadesOrdem` e fica oculta pelo escopo normal. Os testes de lista, admin, F002 e ativação inválida passaram (5/6); a jornada Fiori de Ordens passou com 45/45, mas usa administrador. A suíte unitária manteve uma falha preexistente de expectativa literal versus chave i18n do período. F010 permanece pendente e nenhum commit foi criado.
- 2026-08-11: Adicionado ao controle o status `🟠 Parcialmente resolvido` e aplicado ao F010. O relatório registra que o acesso do criador ao próprio draft foi corrigido e que a regra restante, relativa a `ResponsabilidadesOrdem` após a ativação, foi transferida pelo tutor para uma Change Request do Desafio 3.
- 2026-08-11: F009 confirmado como resolvido. O handler de drafts passou a tratar explicitamente o `PATCH`, combinar o delta com os dados persistidos e usar `req.reject` para impedir períodos inválidos. Sintaxe, compilação CDS e `git diff --check` passaram; a suíte unitária terminou com 7/7 e os testes HTTP de criação e PATCH com 2/2.
- 2026-08-11: Revisada a implementação inicial do F003 em `substituirFiltroBooleano`. A rotina preserva a posição dos conectores lógicos, trata comparações falsas com `not` e percorre expressões CQN aninhadas; foi acrescentado JSDoc técnico. O finding ainda aguarda testes e remoção dos helpers antigos antes de ser encerrado.
- 2026-08-11: O JSDoc do F003 foi refinado com uma definição introdutória: CQN como formato interno de consulta do CAP e `xpr` como representação de um filtro agrupado.
- 2026-08-11: F003 confirmado como resolvido em testes unitários e HTTP/SQLite. A transformação agora substitui o filtro virtual em sua posição original, preservando `and`, `or` e `xpr`; os helpers antigos foram removidos. Sintaxe, compilação CDS e `git diff --check` passaram, a suíte unitária terminou com 8/8 e os cenários focados com 3/3. A suíte HTTP completa teve 25/28, somente com falhas preexistentes de F001, F010 e idioma. A validação HANA foi bloqueada por timeout de conexão antes das asserções.
- 2026-08-11: A validação HANA/HDI do F003 foi repetida após a conexão voltar a responder. O cenário `preserva a semântica de or no filtro virtual no HANA` passou com 1/1, confirmando a correção também no banco-alvo.
- 2026-08-11: F001 implementado com atomicidade por ordem. `liberarOrdemPorID` agora identifica e ordena estoques distintos, adquire os locks, simula todas as reservas com saldo projetado e somente então atualiza os saldos e insere movimentos em lote. `Estoques` recebeu unicidade funcional por material e depósito. Compilação SQL/HANA, sintaxe e `git diff --check` passaram; SQLite focado terminou com 5/5, unitários com 8/8, HANA confirmou atomicidade e concorrência, e a jornada Fiori de Lotes passou com 45/45. A validação da restrição única no HDI aguarda o próximo deploy do modelo.
- 2026-08-11: Após o deploy do F001 no HANA Cloud, a suíte HDI completa passou com 11/11 em 147,638 s. A restrição única foi comprovada no banco e os cenários de mesma ordem concorrente, saldo compartilhado e sequência inversa de locks passaram duas vezes sem deadlock ou consumo duplicado. Na execução instrumentada, os cenários críticos levaram de 7,613 s a 15,880 s end-to-end, incluindo preparação e verificação de fixtures; unitários mantiveram 8/8 e F001 em SQLite 5/5. Os tempos são uma linha de base do Trial, não um teste de carga de produção.
- 2026-08-14: O JSDoc de `aplicarEscopoDeLeitura` foi detalhado para registrar que `V_AcessosOrdem` é correlacionada com a ordem ativa específica consultada; também distingue o bypass administrativo da exceção de propriedade dos drafts novos.
- 2026-08-14: O JSDoc de `aplicarEscopoDeLeitura` recebeu um exemplo com OM-0001 e OM-0002 para esclarecer que cada ordem exige seu próprio vínculo do usuário em `V_AcessosOrdem`.
- 2026-08-14: O JSDoc de `aplicarEscopoDeLeitura` passou a separar explicitamente ordens ativas de drafts novos e a explicar por que a propriedade do draft é avaliada por outra rotina.
- 2026-08-14: Ponto 1 da CR-001 implementado na Object Page de Ordens. O grupo `DadosGerais` passou a usar as chaves graváveis `centro_ID`, `localInstalacao_ID` e `responsavel_matricula`, preservando textos e ajudas de valores. Foi criada a suíte `ordens-annotations.test.js`; metadados 2/2, unitários 10/10, OPA5 48/48 e UI5 linter passaram. Nenhum commit foi criado.
- 2026-08-15: Ponto 2 da CR-001 implementado no escopo de leitura dos drafts. Draft novo agora é correlacionado com `DraftAdministrativeData.CreatedByUser`; o bypass de `admin` foi retirado somente desse escopo, sem substituir o lock nativo por `InProcessByUser`. Sete cenários focados passaram, unitários terminaram com 8/8 e compilações CSN/HANA passaram. A regressão HTTP ficou em 35/37, apenas com as dívidas já conhecidas de responsabilidade após ativação e expectativa de idioma no motivo do cancelamento. Nenhum commit foi criado.
- 2026-08-15: Modelagem de responsabilidades da CR-001 preparada. `ResponsabilidadesOrdem` recebeu unicidade por ordem, usuário e papel; `V_AcessosOrdem` passou a usar `papel` como parte da chave. Compilações CSN/HANA e SQL passaram, os dados iniciais não possuem duplicidades, testes focados terminaram com 2/2 e unitários com 8/8. A regressão HTTP ficou em 37/39, mantendo apenas as duas falhas já conhecidas de ativação e idioma. Nenhum commit foi criado.
- 2026-08-15: Regressão ampliada da modelagem de responsabilidades concluída. Foram adicionados cenários SQLite isolados para combinações distintas de ordem/usuário/papel e para repetição do mesmo vínculo; os quatro testes da modelagem passaram individualmente. A suíte HTTP completa terminou com 39/41, apenas com as duas dívidas já conhecidas de ativação e idioma; unitários 8/8, Ordens OPA5 48/48 e Lotes OPA5 45/45 passaram. Sintaxe, `git diff --check` e compilações SQL/HANA passaram, incluindo a geração do índice único HANA. Concorrência real não foi simulada no SQLite em memória e deve ser confirmada no HANA/HDI após o deploy. Nenhum commit foi criado.
- 2026-08-15: Modelo de responsabilidades implantado no HANA Cloud pelo perfil `hybrid`. O HDI adicionou o artefato `desafio.ordens.ResponsabilidadesOrdem.ordemUsuarioPapel.hdbindex`; o make terminou com sucesso, sem warnings, sem undeploy e sem objetos dependentes redeployados. A ausência de conflito na criação do índice também confirmou que o conteúdo existente não possuía duplicidades incompatíveis.
- 2026-08-15: Regressão HANA/HDI executada após o deploy da modelagem de responsabilidades. A suíte recebeu um 12º cenário que tenta repetir o trio ordem/usuário/papel com outro ID técnico, confirma a rejeição pelo índice único e verifica que somente um vínculo permanece. O teste focado passou com 1/1 e a suíte consolidada terminou com 12/12 em 158,564 s; as fixtures sintéticas foram removidas ao final.
- 2026-08-15: Implementado o bloco de ativação da CR-001. O evento `SAVE` específico de `Ordens.drafts` agora valida a ordem e, para drafts novos, cria `SUPERVISOR` para `DraftAdministrativeData.CreatedByUser` e `EXECUTOR` para `responsavel_matricula` na mesma transação. Foram cobertos criador e responsável iguais, usuários diferentes, responsável inexistente e rollback da ativação; integração SQLite passou com 44/44, suíte geral com 54/54 e OPA5 de Ordens com 48/48. O ESLint isolado não pôde ser usado porque o projeto não possui `eslint.config.*`; `git diff --check`, sintaxe e compilação CDS passaram. Nenhum commit foi criado.
- 2026-08-25: CR-001 concluída. `liberarOrdemPorID` passou a exigir `EXECUTOR` e `cancelarOrdemPorID` passou a exigir `SUPERVISOR`, inclusive para administradores; o lote herdou a autorização por ordem. A carga didática recebeu responsabilidades explícitas para o usuário `admin`, e as suítes SQLite/HANA foram alinhadas à regra. Integração SQLite passou com 51/51, Ordens OPA5 com 48/48, Lotes OPA5 com 45/45, compilações CSN/HANA, sintaxe e `git diff --check` passaram. A repetição da suíte HDI foi bloqueada antes dos testes porque o token do Cloud Foundry expirou; a validação HANA anterior da unicidade permanece registrada. F010 e a CR foram marcados como resolvidos. Nenhum commit foi criado.
